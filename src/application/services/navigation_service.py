...
class NavigationService:
    ...
    def set_current_fragment(self, item_id: str) -> None:
        """Set the current fragment."""
        if not self._project.get_item(item_id):
            raise ValueError(f"Fragmento no encontrado: {item_id}")
        self._current_item_id = item_id
    
    def get_current_fragment(self) -> Optional[MediaItem]:
    ...
    def move_to_previous(self) -> Optional[MediaItem]:
        ...
        prev_fragment = self._project.get_previous_item(self._current_item_id)
        if prev_fragment:
            self._current_item_id = prev_fragment.item_id
